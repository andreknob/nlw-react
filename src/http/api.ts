...
export type GetSubscribersRanking200RankingItem = {
  id: string;
  name: string;
  score: number;
};

export type GetSubscribersRanking200 = {
  ranking: GetSubscribersRanking200RankingItem[];
};
 ...
 */
export const getGetSubscribersRankingUrl = () => {
  return 'http://localhost:3333/subscribers/ranking';
};

export const getSubscribersRanking = async (
  options?: RequestInit
): Promise<GetSubscribersRanking200> => {
  const res = await fetch(getGetSubscribersRankingUrl(), {
    ...options,
    method: 'GET',
  });

  const body = [204, 205, 304].includes(res.status) ? null : await res.text();
  const data: GetSubscribersRanking200 = body ? JSON.parse(body) : {};
